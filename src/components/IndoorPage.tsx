...
const modules = import.meta.glob('../assets/spaces/indoor/**/*.{JPG,jpg,jpeg,png,webp}', { query: '?url', import: 'default', eager: true });
const dynamicImages = Object.values(modules) as string[];

const fallbackImages = [
  '/images/space-indoor.jpg',
  '/images/gallery-5.jpg'
];

const INDOOR_IMAGES = dynamicImages.length > 0 ? dynamicImages : fallbackImages;

export default function IndoorPage({ onClose }: IndoorPageProps) {
  useEffect(() => {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      if (!window.location.hash.includes('indoor')) onClose();
    };
  ...
  return (
    <div className="fixed inset-0 z-[150] w-full h-full overflow-y-auto bg-charcoal text-cream no-scrollbar reveal-fade-in">
      ...
      {/* Hero */}
      <section className="relative w-full space-page-hero h-[50vh] md:h-[75vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <img src="/images/space-indoor.jpg" alt="Luxury Indoor Cafe Maicha" className="w-full h-full object-cover" loading="eager" />
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl reveal-scale-up">
          <span className="text-matcha font-bold text-[10px] md:text-xs tracking-[0.50em] uppercase mb-3 md:mb-4 block">The Cozy Earthy Space</span>
          <h1 className="text-3xl sm:text-6xl md:text-8xl font-display font-bold text-cream mb-4 md:mb-6 tracking-tight leading-none">
            Luxury <span className="italic font-light text-matcha">Indoor</span>
          </h1>
          ...
          <p className="text-cream/80 text-[10px] sm:text-sm md:text-base font-sans tracking-[0.2em] uppercase">
            Earthly Nuance • Deep Conversation • Focused Mind
          </p>
        </div>
      </section>

      {/* Narrative */}
      <section className="relative px-5 md:px-12 py-10 md:py-16 max-w-[1280px] mx-auto text-center reveal-scale-up">
        <span className="text-matcha font-bold text-[10px] tracking-[0.4em] uppercase block mb-3">Indoor Experience</span>
        <h2 className="text-xl md:text-4xl font-display font-bold text-cream mb-4 leading-tight max-w-3xl mx-auto">
          Suasana hangat penuh harmoni yang membuat Anda <span className="italic font-light text-matcha">ingin tinggal lebih lama.</span>
        </h2>
        <div className="w-10 h-[1px] bg-gold/30 mx-auto mb-6" />
        <p className="text-cream/60 text-sm md:text-base lg:text-lg leading-relaxed max-w-3xl mx-auto">
          Ruangan utama Maicha dirancang dengan nuansa earthy—perpaduan warna moka yang tenang, krem hangat yang lembut, serta sentuhan hijau sage yang menyegarkan pandangan.
        </p>
      </section>
      ...
      <InfiniteGallery
        ...
        captionLabel="Maicha Space Portfolio"
        captionTitle="Suasana Estetik Ruang Dalam"
        accentColor="matcha"
      />
      ...
      <section className="relative px-5 md:px-12 py-12 md:py-20 max-w-[1280px] mx-auto">
        <div className="text-center mb-8 md:mb-16">
          <h3 className="text-xl md:text-4xl font-display font-bold text-cream">Kelengkapan Kenyamanan Indoor</h3>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {[
            { title: 'Free WiFi', d: 'Koneksi nirkabel cepat dan stabil untuk penunjang kerja dan berselancar internet.', icon: '📶' },
            { title: 'Musholla', d: 'Fasilitas ibadah yang bersih, tenang, dan nyaman untuk menunaikan shalat.', icon: '🕌' },
            { title: 'Power Outlets', d: 'Colokan listrik di setiap meja untuk menjaga daya laptop.', icon: '⚡' },
            { title: 'Air Purifier', d: 'Sirkulasi udara bersih terus-menerus untuk kenyamanan pernapasan.', icon: '🌿' }
          ].map((item, idx) => (
      ...
      <section className="relative px-5 md:px-12 py-16 md:py-24 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto space-y-5 md:space-y-8">
          <h3 className="text-2xl md:text-5xl font-display font-bold text-cream tracking-tight">
            Kunjungi Luxury Indoor<br />
            <span className="italic font-light text-matcha">Maicha Coffee Hari Ini</span>
          </h3>
          <p className="text-cream/55 text-sm md:text-base max-w-lg mx-auto">
            Rasakan kehangatan kopi pilihan dan hidangan premium di sofa ternyaman kami.
          </p>
          <a
            href="https://wa.me/6281222847595?text=Halo%20Maicha%2C%20saya%20tertarik%20menanyakan%20ketersediaan%20tempat%20di%20Luxury%20Indoor"
